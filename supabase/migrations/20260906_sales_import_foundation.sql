...
alter table public.sales
  add column if not exists source_batch_id uuid references public.sales_import_batches(id) on delete set null;
...
create index if not exists idx_sales_source_batch
  on public.sales(source_batch_id);

create index if not exists idx_sales_brand_sold_at
  on public.sales(brand_id, sold_at);

create index if not exists idx_sales_import_batches_brand_created
  on public.sales_import_batches(brand_id, created_at desc);
...
create policy brand_read_sales_import_batches
...
create policy brand_write_sales_import_batches
  on public.sales_import_batches
