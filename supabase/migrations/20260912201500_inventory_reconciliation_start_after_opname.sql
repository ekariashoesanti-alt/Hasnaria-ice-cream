DROP VIEW IF EXISTS public.inventory_stock_reconciliation;

CREATE VIEW public.inventory_stock_reconciliation
...
purchase_totals AS (
  SELECT
    ...
    CASE WHEN b.tracking_active THEN
      COALESCE(SUM(l.qty) FILTER (WHERE l.purchase_date > b.baseline_date), 0)
...
COMMENT ON VIEW public.inventory_stock_reconciliation IS
  'Reconciled stock. Tracking starts after the first physical stock opname. Latest physical opname is the baseline; later purchases add stock and mapped sales usage subtracts it.';
