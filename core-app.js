(function () {
  ...
  function render() {
    ...
    var purchaseDataYears = Object.keys(purchaseMonth).map(function(m){return m.slice(0,4);}).filter(function(y){return /^\d{4}$/.test(y);}).sort();
    var purchaseSelectedYear = purchaseChartYear || purchaseDataYears[purchaseDataYears.length-1] || today().slice(0,4);
    var purchaseYears = Array.from(new Set(purchaseDataYears.concat([today().slice(0,4),purchaseSelectedYear]))).sort().reverse();
    var purchaseMonths = Array.from({length:12},function(_,i){return purchaseSelectedYear+"-"+String(i+1).padStart(2,"0");});
    var purchaseMonthLabels = ["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];
    var purchaseMax = purchaseMonths.reduce(function(a,m){return Math.max(a,purchaseMonth[m]||0);},0);
    ...
    function purchaseHeaderRow(rows){for(var i=0;i<Math.min(rows.length,20);i++){var h=rows[i]||[],hasDate=purchaseCol(h,[/^tgl$|tanggal|date/])>=0,hasItem=purchaseCol(h,[/nama.*bahan|nama.*barang|^bahan$|^item$|^produk$/])>=0,hasAmount=purchaseCol(h,[/tunai|cash|paylater|utang|qris|transfer|total|amount|harga.*satuan/])>=0;if(hasDate&&hasItem&&hasAmount)return i;}return -1;}
    async function purchaseParseFile(f){if(!window.XLSX)throw new Error("Pembaca Excel belum siap. Tunggu sebentar lalu coba lagi.");var wb=XLSX.read(await f.arrayBuffer(),{type:"array",cellDates:true}),out=[],preferred=wb.SheetNames.filter(function(n){return String(n).trim().toUpperCase()==="BELANJA";}),sheets=preferred.length?preferred:wb.SheetNames;sheets.forEach(function(sheet,si){var rows=XLSX.utils.sheet_to_json(wb.Sheets[sheet],{header:1,defval:"",raw:true}),hi=purchaseHeaderRow(rows);if(hi<0)return;var h=rows[hi],di=purchaseCol(h,[/^tgl$|tanggal|date/]),ii=purchaseCol(h,[/nama.*bahan|nama.*barang|^bahan$|^item$|^produk$/]),qi=purchaseCol(h,[/^jumlah$|qty|quantity/]),ui=purchaseCol(h,[/harga.*satuan|unit price|^harga$/]),ti=purchaseCol(h,[/^total$|amount/]),paymentCols=[];h.forEach(function(x,i){if(/tunai|cash|paylater|utang|qris|transfer/i.test(String(x||"")))paymentCols.push(i);});var lastDate=null;rows.slice(hi+1).forEach(function(r,ri){var parsed=di>=0?purchaseParseDate(r[di]):null;if(parsed)lastDate=parsed;var item=ii>=0?String(r[ii]||"").trim():"";if(!item||/^total$/i.test(item)||!lastDate)return;var qty=qi>=0?purchaseParseNumber(r[qi]):0,u=ui>=0?purchaseParseNumber(r[ui]):0,payTotal=0,payNames=[];paymentCols.forEach(function(pi){var val=purchaseParseNumber(r[pi]);if(val){payTotal+=val;payNames.push(String(h[pi]||"").trim());}});var t=payTotal||(ti>=0?purchaseParseNumber(r[ti]):0)||(qty&&u?qty*u:0);if(!(t>0)&&!(qty>0))return;out.push({brand_id:BRAND,source_period:lastDate.slice(0,7)+"-01",source_file:f.name,row_no:(si+1)*100000+hi+ri+2,purchase_date:lastDate,item_name:item,quantity_text:qty?String(qty):String(qi>=0?r[qi]||"":""),unit_text:"",unit_price:u,total_amount:t,payment_method:payNames.join(" + ")||(t>0?"Tidak dirinci":""),notes:"Sheet: "+sheet,raw_data:{sheet:sheet,excel_row:hi+ri+2,headers:h,row:r}});});});return out;}
    var importTotal=purchaseImportRows.reduce(function(a,x){return a+Number(x.total_amount||0);},0),latestSource=purchaseImportRows.length?purchaseImportRows[0].source_file:"Belum ada file",latestPeriod=purchaseImportRows.length?String(purchaseImportRows[0].source_period||"").slice(0,7):"—";
    $("pembelian").innerHTML =
      '<div class="card"><h2>Data Pembelian</h2><p class="small">File Excel menjadi sumber data ringkasan, grafik, dan riwayat pembelian di halaman ini.</p><div class="grid-3"><div><div class="label">Total dari file</div><div class="metric">'+rp(importTotal)+'</div></div><div><div class="label">Transaksi terbaca</div><div class="metric">'+purchaseImportRows.length.toLocaleString("id-ID")+'</div></div><div><div class="label">Periode terbaru</div><div class="metric">'+esc(latestPeriod)+'</div><div class="small">'+esc(latestSource)+'</div></div></div></div>'+
      '<div class="card"><h2>Upload File Pembelian</h2><p class="small">Mendukung Excel/CSV. Untuk file Rencana Belanja, sistem otomatis membaca sheet BELANJA, meneruskan tanggal yang kosong, dan menjumlahkan Tunai, PayLater, serta Utang Ria.</p><input id="purchaseFile" type="file" accept=".xlsx,.xls,.csv" hidden><button class="primary" id="purchaseUploadBtn" '+(canStock(role)?'':'disabled')+'>Pilih file Excel</button> <span id="purchaseFileName" class="small"></span><div id="purchaseUploadStatus" class="small"></div><div id="purchasePreview"></div></div>'+
      '<div class="card"><h2>Tren Pembelian Tahunan</h2><label for="purchaseYear">Tahun</label><select id="purchaseYear">'+purchaseYears.map(function(y){return '<option value="'+y+'"'+(y===purchaseSelectedYear?' selected':'')+'>'+y+'</option>';}).join('')+'</select><p class="small">Januari–Desember · Bulan tanpa data ditampilkan kosong.</p><div style="overflow:auto"><div id="purchaseAnnualChart" style="display:flex;align-items:flex-end;gap:10px;height:210px">'+purchaseMonths.map(function(m,i){var hasData=Object.prototype.hasOwnProperty.call(purchaseMonth,m),v=hasData?purchaseMonth[m]:null,h=hasData&&v>0&&purchaseMax>0?Math.max(1,Math.round(v/purchaseMax*160)):0;return '<div style="flex:1;text-align:center;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center"><div class="small">'+(hasData?rp(v):'')+'</div>'+(h>0?'<div data-purchase-height="'+h+'"></div>':'')+'<div class="small">'+purchaseMonthLabels[i]+'</div></div>';}).join('')+'</div></div></div>'+
      '<div class="card"><h2>Riwayat dari File</h2><div style="overflow:auto"><table><thead><tr><th>Tanggal</th><th>Nama bahan</th><th>Jumlah</th><th>Harga satuan</th><th>Total</th><th>Pembayaran</th></tr></thead><tbody>'+(purchaseImportRows.slice(0,100).map(function(x){return '<tr><td>'+esc(x.purchase_date||'—')+'</td><td>'+esc(x.item_name||'—')+'</td><td>'+esc(x.quantity_text||'—')+'</td><td>'+rp(x.unit_price)+'</td><td>'+rp(x.total_amount)+'</td><td>'+esc(x.payment_method||'—')+'</td></tr>';}).join('')||'<tr><td colspan="6" class="small">Belum ada data dari file.</td></tr>')+'</tbody></table></div></div>'+
      '<div class="card"><h2>Pengadaan Manual</h2><div class="grid-3"><div><div class="label">Total pengadaan · 90 hari</div><div class="metric">' + rp(pTotal) + '</div></div><div><div class="label">Investasi & aset</div><div class="metric">' + rp(pInv) + '</div></div><div><div class="label">Admin + Kepegawaian</div><div class="metric">' + rp(pAdmin + pHr) + '</div></div></div></div>' +
    ...
    $("purchaseAnnualChart").style.minWidth="960px";
    $("purchaseYear").onchange=function(){purchaseChartYear=this.value;render();};
    ...
    if($("purchaseUploadBtn"))$("purchaseUploadBtn").onclick=function(){$("purchaseFile").click();};
    if($("purchaseFile"))$("purchaseFile").onchange=async function(){var f=this.files&&this.files[0];if(!f)return;$("purchaseFileName").textContent=f.name;$("purchaseUploadStatus").textContent="Membaca file…";$("purchasePreview").innerHTML="";try{var rows=await purchaseParseFile(f);if(!rows.length)throw new Error("Data pembelian tidak ditemukan. Pastikan file memiliki kolom TGL, NAMA BAHAN, JUMLAH, dan nilai pembayaran.");var total=rows.reduce(function(a,x){return a+Number(x.total_amount||0);},0);$("purchaseUploadStatus").textContent="File siap diperiksa.";$("purchasePreview").innerHTML='<div class="okbox"><b>'+rows.length.toLocaleString("id-ID")+' transaksi terbaca · '+rp(total)+'</b><div class="small">Contoh: '+rows.slice(0,4).map(function(x){return esc(x.purchase_date)+' · '+esc(x.item_name)+' · '+rp(x.total_amount);}).join('<br>')+'</div><button class="primary" id="purchaseCommit">Gunakan sebagai data Pembelian</button></div>';$("purchaseCommit").onclick=async function(){this.disabled=true;$("purchaseUploadStatus").textContent="Menyimpan 0/"+rows.length+"…";try{for(var z=0;z<rows.length;z+=150){var rr=await db.from("offline_purchase_history").upsert(rows.slice(z,z+150),{onConflict:"brand_id,source_file,row_no"});if(rr.error)throw rr.error;$("purchaseUploadStatus").textContent="Menyimpan "+Math.min(z+150,rows.length)+"/"+rows.length+"…";}$("purchaseUploadStatus").textContent="Upload berhasil. Data Pembelian sudah diperbarui.";await loadAll();render();}catch(e){$("purchaseUploadStatus").textContent="Gagal menyimpan: "+e.message;this.disabled=false;}};}catch(e){$("purchaseUploadStatus").textContent="Gagal membaca: "+e.message;}};
