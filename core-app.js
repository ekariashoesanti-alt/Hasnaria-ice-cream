(function () {
  ...
  function render() {
    ...
    $("pembelian").insertAdjacentHTML("afterbegin", '<div class="card"><h2>Rekap Hasil Upload</h2><div style="overflow:auto"><table><thead><tr><th>Tanggal</th><th>Sheet</th><th>Item</th><th>Qty</th><th>Harga</th><th>Total</th><th>Pembayaran</th><th>Sumber</th></tr></thead><tbody>'+(purchaseImportRows.slice(0,500).map(function(x){var r=x.raw_data||{};return "<tr><td>"+esc(x.purchase_date||"—")+"</td><td>"+esc(r.sheet||"—")+"</td><td>"+esc(x.item_name||"—")+"</td><td>"+esc(x.quantity_text||"—")+"</td><td>"+rp(x.unit_price||0)+"</td><td>"+rp(x.total_amount||0)+"</td><td>"+esc(x.payment_method||"—")+"</td><td>"+esc(x.source_file||"—")+"</td></tr>";}).join("")||'<tr><td colspan="8" class="small">Belum ada hasil upload.</td></tr>')+"</tbody></table></div></div>');
    ...
    async function purchaseParseFile(f){var wb=XLSX.read(await f.arrayBuffer(),{type:"array",cellDates:true}),out=[];wb.SheetNames.forEach(function(sheet){var rows=XLSX.utils.sheet_to_json(wb.Sheets[sheet],{header:1,defval:"",raw:true});if(!rows.length)return;var h=rows[0],di=purchaseCol(h,[/tanggal|tgl|date/]),ii=purchaseCol(h,[/nama|item|bahan|barang|produk/]),qi=purchaseCol(h,[/qty|quantity|jumlah/]),ui=purchaseCol(h,[/harga.*satuan|unit price|^harga$/]),ti=purchaseCol(h,[/total|amount/]),pi=purchaseCol(h,[/payment|pembayaran|tunai|cash|qris|transfer|paylater/]);rows.slice(1).forEach(function(r,ri){var d=di>=0?purchaseParseDate(r[di]):null,item=ii>=0?String(r[ii]||"").trim():"",q=qi>=0?String(r[qi]||"").trim():"",u=ui>=0?purchaseParseNumber(r[ui]):0,t=ti>=0?purchaseParseNumber(r[ti]):(u&&purchaseParseNumber(r[qi])?u*purchaseParseNumber(r[qi]):0),pay=pi>=0?String(r[pi]||"").trim():"";if(d&&(item||t>0))out.push({brand_id:BRAND,source_period:d.slice(0,7)+"-01",source_file:f.name,row_no:ri+2,purchase_date:d,item_name:item||("Baris "+(ri+2)),quantity_text:q,unit_text:"",unit_price:u,total_amount:t,payment_method:pay,notes:"Sheet: "+sheet,raw_data:{sheet:sheet,headers:h,row:r}});});});return out;}
    if($("purchaseUploadBtn"))$("purchaseUploadBtn").onclick=function(){$("purchaseFile").click();};
    if($("purchaseFile"))$("purchaseFile").onchange=async function(){var f=this.files&&this.files[0];if(!f)return;$("purchaseFileName").textContent=f.name;$("purchaseUploadStatus").textContent="Membaca seluruh sheet…";try{var rows=await purchaseParseFile(f);if(!rows.length)throw new Error("Tidak ditemukan data pembelian yang dapat dikenali.");$("purchasePreview").innerHTML='<div class="warnbox"><b>Preview</b> · '+rows.length.toLocaleString("id-ID")+' baris terdeteksi.<br><button class="primary" id="purchaseCommit">Simpan ke database</button></div>';$("purchaseCommit").onclick=async function(){this.disabled=true;try{for(var z=0;z<rows.length;z+=150){var rr=await db.from("offline_purchase_history").upsert(rows.slice(z,z+150),{onConflict:"brand_id,source_file,row_no"});if(rr.error)throw rr.error;$("purchaseUploadStatus").textContent="Menyimpan "+Math.min(z+150,rows.length)+"/"+rows.length+" ("+Math.round(Math.min(z+150,rows.length)/rows.length*100)+"%)…";} $("purchaseUploadStatus").textContent="✓ Upload berhasil tersimpan.";await loadAll();render();}catch(e){$("purchaseUploadStatus").textContent="Gagal menyimpan: "+e.message;this.disabled=false;}};}catch(e){$("purchaseUploadStatus").textContent="Gagal membaca: "+e.message;}};
    $("pembelian").innerHTML =
