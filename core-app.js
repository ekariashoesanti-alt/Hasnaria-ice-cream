(function () {
  ...
  function render() {
    ...
    $("pembelian").innerHTML =
      ...
      '<div class="card"><h2>Upload File Pembelian</h2><p class="small">Mendukung Excel/CSV. Untuk file Rencana Belanja, sistem otomatis membaca sheet BELANJA, meneruskan tanggal yang kosong, dan menjumlahkan Tunai, PayLater, serta Utang Ria.</p><input id="purchaseFile" type="file" accept=".xlsx,.xls,.csv" hidden><div style="display:grid;grid-template-columns:minmax(170px,auto) minmax(260px,1fr) minmax(150px,220px);gap:12px;align-items:end;margin-top:14px"><button class="primary" id="purchaseUploadBtn" '+(canStock(role)?'':'disabled')+' style="height:48px">Pilih file Excel</button><div id="purchaseFileName" class="small" style="height:48px;border:1px solid var(--bd);border-radius:10px;padding:0 14px;display:flex;align-items:center;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">Belum ada file dipilih</div><div><label for="purchaseYear">Tahun</label><select id="purchaseYear" style="height:48px;margin-top:4px">'+purchaseYears.map(function(y){return '<option value="'+y+'"'+(y===purchaseSelectedYear?' selected':'')+'>'+y+'</option>';}).join('')+'</select></div></div><div id="purchaseUploadStatus" class="small" style="margin-top:8px"></div><div id="purchasePreview"></div></div>'+
      '<div class="card"><h2>Tren Pembelian Tahunan</h2><p class="small">Januari–Desember · Bulan tanpa data ditampilkan kosong.</p><div style="overflow:auto"><div id="purchaseAnnualChart" style="display:flex;align-items:flex-end;gap:10px;height:210px">'+purchaseMonths.map(function(m,i){var hasData=Object.prototype.hasOwnProperty.call(purchaseMonth,m),v=hasData?purchaseMonth[m]:null,h=hasData&&v>0&&purchaseMax>0?Math.max(1,Math.round(v/purchaseMax*160)):0;return '<div style="flex:1;text-align:center;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center"><div class="small">'+(hasData?rp(v):'')+'</div>'+(h>0?'<div data-purchase-height="'+h+'"></div>':'')+'<div class="small">'+purchaseMonthLabels[i]+'</div></div>';}).join('')+'</div></div></div>'+
