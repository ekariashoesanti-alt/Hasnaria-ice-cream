(function () {
  ...
  if (typeof supabase !== 'undefined' && AUTH_URL && AUTH_KEY) {
    ...
    try {
      ...
      var passwordPolicy = window.__HASNARIA_PASSWORD_POLICY || { validate:function(password){var value=String(password||'');var ok=value.length>=12&&/[a-z]/.test(value)&&/[A-Z]/.test(value)&&/[0-9]/.test(value)&&/[^A-Za-z0-9]/.test(value);return {ok:ok,message:ok?'':'Password baru minimal 12 karakter dan wajib memuat huruf kecil, huruf besar, angka, serta simbol.'};} };
      function passwordPolicyError(password){var result=passwordPolicy.validate(password);if(result&&!result.ok){var err=new Error(result.message||'Password baru tidak memenuhi kebijakan keamanan.');err.name='HasnariaPasswordPolicyError';return err;}return null;}
      var originalSignUp=sharedAuthClient.auth.signUp.bind(sharedAuthClient.auth);
      sharedAuthClient.auth.signUp=function(credentials){var err=credentials&&Object.prototype.hasOwnProperty.call(credentials,'password')?passwordPolicyError(credentials.password):null;if(err)return Promise.resolve({data:{user:null,session:null},error:err});return originalSignUp(credentials);};
      var originalUpdateUser=sharedAuthClient.auth.updateUser.bind(sharedAuthClient.auth);
      sharedAuthClient.auth.updateUser=function(attributes,options){var err=attributes&&Object.prototype.hasOwnProperty.call(attributes,'password')?passwordPolicyError(attributes.password):null;if(err)return Promise.resolve({data:{user:null},error:err});return originalUpdateUser(attributes,options);};
      var nativeFetch=window.fetch.bind(window),authRefreshPromise=null;
      function isSupabaseRestRequest(input){try{return String(typeof input==='string'?input:(input&&input.url)||'').indexOf(String(AUTH_URL)+'/rest/v1/')===0;}catch(_){return false;}}
      function refreshAuthSession(){if(authRefreshPromise)return authRefreshPromise;authRefreshPromise=sharedAuthClient.auth.refreshSession().then(function(res){if(res&&res.error)throw res.error;return res&&res.data&&res.data.session?res.data.session:null;}).finally(function(){authRefreshPromise=null;});return authRefreshPromise;}
      window.fetch=async function(input,init){var res=await nativeFetch(input,init);if(!isSupabaseRestRequest(input)||res.ok)return res;var body='';try{body=await res.clone().text();}catch(_){}if(!/PGREST303|JWT issued at future/i.test(body))return res;try{var session=await refreshAuthSession();if(!session||!session.access_token)return res;var retryInit=Object.assign({},init||{}),headers=new Headers((init&&init.headers)||{});headers.set('Authorization','Bearer '+session.access_token);retryInit.headers=headers;return await nativeFetch(input,retryInit);}catch(_){return res;}};
