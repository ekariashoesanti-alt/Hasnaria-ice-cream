  ...
  async function run(fileOrFiles,ctx){
    ...
    try{
      ...
      try{
        ...
        var daily=[];var cursor=new Date(from+'T00:00:00Z'),endCursor=new Date(to+'T00:00:00Z');
        while(cursor<=endCursor){var ds=cursor.toISOString().slice(0,10);var d=days[ds]||{omzet:0,trx:0,cash:0,qris:0,tf:0,skus:{}};daily.push({brand_id:BRAND,metric_date:ds,cash_revenue:Math.round(d.omzet),transactions:d.trx,notes:makeDailyNotes(oldMap[ds],d)});cursor.setUTCDate(cursor.getUTCDate()+1);}
