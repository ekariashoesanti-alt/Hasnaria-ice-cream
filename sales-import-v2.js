...
(function () {
  ...
  async function run(fileOrFiles,ctx){
    ...
    try{
      ...
      var hashes=[];for(var fh=0;fh<files.length;fh++)hashes.push(await sha256(files[fh]));var fileHash=hashes.join(','),dup=await api('sales_import_batches?brand_id=eq.'+encodeURIComponent(BRAND)+'&file_hash=eq.'+encodeURIComponent(fileHash)+'&status=eq.completed&select=id&limit=1')||[];if(dup.length)throw new Error('File yang sama sudah pernah berhasil diimpor. Import dibatalkan untuk mencegah transaksi ganda.');
      ...
      try{
        var salesRows=active.map(function(g){return{brand_id:BRAND,external_transaction_id:g.id,source_batch_id:batchId,sold_at:g.datetime||g.date,sold_hour:g.hour,transaction_count:1,total_amount:Math.round(g.total||0),cash_amount:Math.round(g.cash||0),qris_amount:Math.round(g.qris||0),tf_amount:Math.round(g.tf||0),channel:'majoo',notes:g.status?'status='+g.status:null};});
