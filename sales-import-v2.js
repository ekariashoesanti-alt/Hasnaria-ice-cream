  ...
  async function run(fileOrFiles,ctx){
    ...
    try{
      ...
      try{
        ...
        /* TRUE REPLACE for overlapping Majoo period: remove old Majoo transactions in the uploaded date range first. Manual/non-Majoo sales are untouched. */
        var oldPeriod=await api('sales?brand_id=eq.'+encodeURIComponent(BRAND)+'&channel=eq.majoo&sold_at=gte.'+encodeURIComponent(from)+'&sold_at=lte.'+encodeURIComponent(to)+'&select=id&limit=10000')||[];
        var oldIds=oldPeriod.map(function(s){return s.id;});
        for(var odi=0;odi<oldIds.length;odi+=60){var od=oldIds.slice(odi,odi+60);await api('sale_items?sale_id=in.('+od.map(encodeURIComponent).join(',')+')',{method:'DELETE',headers:{Prefer:'return=minimal'}});}
        for(var odi2=0;odi2<oldIds.length;odi2+=60){var od2=oldIds.slice(odi2,odi2+60);await api('sales?id=in.('+od2.map(encodeURIComponent).join(',')+')',{method:'DELETE',headers:{Prefer:'return=minimal'}});}
        var saved=await api('sales',{method:'POST',headers:{'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify(salesRows)})||[],saleByExt={};saved.forEach(function(s){saleByExt[s.external_transaction_id]=s;});
        ...
        var old=await api('daily_metrics?brand_id=eq.'+encodeURIComponent(BRAND)+'&metric_date=gte.'+from+'&metric_date=lte.'+to+'&select=metric_date,notes&limit=5000')||[],oldMap={};old.forEach(function(r){oldMap[r.metric_date]=r.notes||'';});
        /* Rebuild the entire uploaded date range so dates absent from the new file cannot retain stale sales totals. */
        var daily=[];var cursor=new Date(from+'T00:00:00+07:00'),endCursor=new Date(to+'T00:00:00+07:00');
        while(cursor<=endCursor){var ds=cursor.toISOString().slice(0,10);var d=days[ds]||{omzet:0,trx:0,cash:0,qris:0,tf:0,skus:{}};daily.push({brand_id:BRAND,metric_date:ds,cash_revenue:Math.round(d.omzet),transactions:d.trx,notes:makeDailyNotes(oldMap[ds],d)});cursor.setDate(cursor.getDate()+1);}
