...
`;
  ...
  function removeWorstEverywhere() {
    ...
    if (!host) return;
    Array.prototype.forEach.call(host.querySelectorAll('h1,h2,h3,h4,h5,h6,div,p,span,strong,b'), function (el) {
      if ((el.textContent || '').replace(/\s+/g, ' ').trim() !== '5 Worst Performer (Qty)') return;
      var panel = el.closest('.sb-card,.card,.panel,.box,.section') || el.parentElement;
      if (panel && panel !== host) panel.remove();
      else el.remove();
    });
  }

  function fit() {
    var host = document.getElementById('sales'), board = host && host.querySelector('.sale-board');
