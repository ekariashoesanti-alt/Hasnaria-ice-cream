...
/* Worst Performer: oculto apenas pela regra abaixo até ser reintroduzido no layout. */
#sales .sales-worst-performer{display:none!important}
...
`;
  ...
  function splitPerformerBoxes(){var host=document.getElementById('sales');if(!host||host.classList.contains('hidden'))return;var grids=host.querySelectorAll('.sb-grid-main');for(var gi=0;gi<grids.length;gi++){var grid=grids[gi];if(grid.getAttribute('data-performers-split')==='1')continue;var cards=Array.prototype.slice.call(grid.querySelectorAll(':scope > .sb-card'));var performerCard=null;cards.some(function(c){if(findHeading(c,'Top 5 Seller (Qty)')||findHeading(c,'5 Worst Performer (Qty)')){performerCard=c;return true;}return false;});if(!performerCard)continue;var top=findHeading(performerCard,'Top 5 Seller (Qty)'),worst=findHeading(performerCard,'5 Worst Performer (Qty)');if(!top||!worst)continue;var stack=document.createElement('div');stack.className='sales-right-stack';function makeCard(heading,extra){var c=document.createElement('div');c.className='sb-card '+(extra||'');sectionParts(performerCard,heading).forEach(function(node){c.appendChild(node);});return c;}stack.appendChild(makeCard(top));stack.appendChild(makeCard(worst,'sales-worst-performer'));performerCard.replaceWith(stack);grid.setAttribute('data-performers-split','1');}}
  function restoreWorstBox(){var host=document.getElementById('sales');if(!host)return;var worst=host.querySelector('.sales-worst-performer');if(!worst)return;worst.style.display='block';var stack=worst.parentElement;var top=stack&&stack.querySelector(':scope > .sb-card:not(.sales-worst-performer)');if(top){var h=top.getBoundingClientRect().height;worst.style.height=Math.ceil(h)+'px';worst.style.minHeight=Math.ceil(h)+'px';worst.style.width='100%';}}
