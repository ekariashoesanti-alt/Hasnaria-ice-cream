...
/* Pastikan Worst Performer benar-benar hilang dari DOM/tampilan jika masih dirender oleh board. */
#sales .sales-right-stack .sb-card:has(h1,h2,h3,h4,h5,h6){ }
...
`;
  ...
  function removeWorstEverywhere(){var host=document.getElementById('sales');if(!host)return;Array.prototype.forEach.call(host.querySelectorAll('h1,h2,h3,h4,h5,h6,div,p,span,strong,b'),function(el){if(textOf(el)!=='5 Worst Performer (Qty)')return;var panel=el.closest('.sb-card,.card,.panel,.box,.section')||el.parentElement;if(panel&&panel!==host){panel.remove();}else{el.remove();}});}
  function splitPerformerBoxes(){var host=document.getElementById('sales');if(!host||host.classList.contains('hidden'))return;var grids=host.querySelectorAll('.sb-grid-main');for(var gi=0;gi<grids.length;gi++){var grid=grids[gi];if(grid.getAttribute('data-performers-split')==='1')continue;var cards=Array.prototype.slice.call(grid.querySelectorAll(':scope > .sb-card'));var performerCard=null;cards.some(function(c){if(findHeading(c,'Top 5 Seller (Qty)')||findHeading(c,'5 Worst Performer (Qty)')){performerCard=c;return true;}return false;});if(!performerCard)continue;var top=findHeading(performerCard,'Top 5 Seller (Qty)'),worst=findHeading(performerCard,'5 Worst Performer (Qty)');if(!top||!worst)continue;var stack=document.createElement('div');stack.className='sales-right-stack';function makeCard(heading){var c=document.createElement('div');c.className='sb-card';var head=heading.closest('.sb-card-head')||heading.parentElement;c.appendChild(head);var n=head.nextElementSibling;while(n){if(n.classList&&n.classList.contains('sb-card-head'))break;if(n.classList&&(n.classList.contains('sb-live-note')||n.classList.contains('sb-legacy-note')))break;var next=n.nextElementSibling;c.appendChild(n);n=next;}return c;}stack.appendChild(makeCard(top));stack.appendChild(makeCard(worst));performerCard.replaceWith(stack);grid.setAttribute('data-performers-split','1');}}
  ...
  function run(){injectStyle();hideSalesPanels();requestAnimationFrame(function(){removeWorstEverywhere();splitPerformerBoxes();removeWorstEverywhere();fit();});}
