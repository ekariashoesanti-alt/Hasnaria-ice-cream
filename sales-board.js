...
(function () {
  ...
  var STATE = { mode: 'daily', rows: [], loading: false, importing: false, fileName: '', msg: '', error: '', draw: false };
  ...
  async function loadMetrics() {
    ...
    try {
      ...
      STATE.loading = false; STATE.error = ''; draw();
    } catch (e) { STATE.loading = false; STATE.error = e.message; draw(); }
  }
  ...
  function draw() {
    ...
    html += '<section class="sb-card sb-import"><div class="sb-card-head"><div><h3>Update dari Majoo</h3><span>Upload laporan untuk mengganti data produk per tanggal tanpa menambah duplikat</span></div></div>' +
      '<div class="sb-import-copy"><b>Format diterima</b><div>.csv · .xlsx · .xls · .txt</div><p>Kolom fleksibel: tanggal/tgl/date, total penjualan/omzet/penjualan/grand total, transaksi/jumlah struk, nama produk/item/sku, qty, tunai/cash, qris, transfer.</p></div>' + renderImportBlock() + (STATE.msg ? '<div class="sb-ok">' + esc(STATE.msg) + '</div>' : '') + '</section></div>';
  ...
  function bind() {
    ...
    host.querySelectorAll('#sbFormat').forEach(function (b) { b.onclick = function () { alert('Gunakan export laporan penjualan Majoo yang berisi tanggal dan minimal omzet/penjualan. Untuk product mix, sertakan nama produk/item/SKU + qty. Pembayaran opsional: tunai/cash, qris, transfer.'); }; });
  ...
  function normalizeImport(rows) {
    ...
    var kDate = findKey(keys, [/^(tanggal|tgl|date)$/,/tanggal/,/date/]);
    var kTotal = findKey(keys, [/total penjualan/,/grand total/,/omzet/,/penjualan/,/total/]);
    ...
    if (!kDate) throw new Error('Kolom tanggal/tgl/date tidak ditemukan.');
    if (!kTotal) throw new Error('Kolom omzet/total penjualan/grand total tidak ditemukan.');
    var groups = {};
    rows.forEach(function(row){
      var date = parseDate(row[kDate]); if (!date) return;
      var g = groups[date] || (groups[date] = { date:date, omzet:0, trx:0, trxVals:[], cash:[], qris:[], tf:[], skus:{} });
      var rev = num(row[kTotal]); if (rev != null) g.omzet += rev;
    ...
    var out = Object.keys(groups).sort().map(function(date){
      ...
      var sameRevRows = rows.filter(function(r){ return parseDate(r[kDate])===date; }).map(function(r){return num(r[kTotal]);}).filter(function(v){return v!=null;});
  ...
  document.addEventListener('click', function(e){ var t=e.target&&e.target.closest?e.target.closest('[data-tab="sales"],.tab'):null; if(!t)return; var id=t.getAttribute('data-tab'); if(id==='sales'||(t.textContent||'').toLowerCase().indexOf('penjualan')!==-1){ setTimeout(mount,40); setTimeout(function(){if(!STATE.rows.length)loadMetrics();},220); } }, true);
  var tries=0; (function wait(){ mount(); if(document.getElementById('sales') && !STATE.rows.length && tok()) loadMetrics(); if(tries++<300)setTimeout(wait,300); })();
})();
