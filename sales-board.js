...
(function () {
  ...
  function headerish(cells) {
    ...
    var hasRev = n.some(function (x) { return /^(penjualan|omzet)/.test(x) || x.indexOf('penjualan rp') >= 0; });
    var hasSku = n.some(function (x) { return /^(produk|sku|item|nama produk)/.test(x); });
    var hits = n.filter(function (x) {
      return /^(periode|tanggal|tgl|date|hari|period|penjualan|omzet|transaksi|produk|sku|jumlah|qty|laba)/.test(x);
    }).length;
