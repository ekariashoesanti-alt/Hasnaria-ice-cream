/* Hasnaria sales-board multipart loader v43 — normalized Majoo sales foundation */
(function () {
  'use strict';
  var PARTS = [
  ...
  function showError(msg) {
    var host=document.getElementById('sales');if(!host){console.error('Hasnaria sales-board load failed:',msg);return;}
  ...
  function ensureSalesTab(){
    var tabs=document.getElementById('tabs');if(!tabs)return false;var btn=tabs.querySelector('[data-tab="sales"]');
    ...
    btn.style.display='';btn.textContent='Penjualan';btn.onclick=function(){['dashboard','sales','ops','stok','shift','social','approval','team','sistem'].forEach(function(id){var el=document.getElementById(id);if(el)el.classList.toggle('hidden',id!=='sales');});tabs.querySelectorAll('.tab').forEach(function(b){b.classList.toggle('on',b===btn);});setTimeout(function(){if(typeof window.__hasnariaReloadSales==='function')window.__hasnariaReloadSales();},80);};return true;
  }
  ...
  function patchSource(code){
    code=code.replace(/function revenue\(v\) \{ var n=num\(v\); if\(n==null\) return null; return \(n>0 && n<10000\) \? n\*1000 : n; \}/,'function revenue(v) { var n=num(v); return n==null ? null : n; }');
    code=code.replace("async function ensureXLSX() {\n    if (window.XLSX) return;","async function ensureXLSX() {\n    if (window.XLSX) return;\n    if (window.__HASNARIA_XLSX_READY) { await window.__HASNARIA_XLSX_READY; if (window.XLSX) return; }");
    code=code.replace(/async function importFiles\(fileOrFiles\) \{[\s\S]*?\n  function importFile\(f\) \{ return importFiles\(f\); \}/,
      "async function importFiles(fileOrFiles) {\n    if (typeof window.__HASNARIA_IMPORT_V2 !== 'function') throw new Error('Importer Majoo Phase 2 belum siap. Silakan refresh halaman.');\n    return window.__HASNARIA_IMPORT_V2(fileOrFiles, { STATE: STATE, BRAND: BRAND, SB: SB, KEY: KEY, setImportStatus: setImportStatus, draw: draw, getFileMatrix: getFileMatrix, loadMetrics: loadMetrics, getTok: getTok });\n  }\n\n  function importFile(f) { return importFiles(f); }");
    return code;
  }
  Promise.all(PARTS.map(function(u){return fetch(u,{cache:'no-cache'}).then(function(r){if(!r.ok)throw new Error(u+' HTTP '+r.status);return r.text();});}).concat([
    fetch('/sales-import-v2.js?v=1',{cache:'no-cache'}).then(function(r){if(!r.ok)throw new Error('/sales-import-v2.js HTTP '+r.status);return r.text();})
  ]))
    .then(function(chunks){var code=chunks.join('');if(code.indexOf('Hasnaria Sales')<0)throw new Error('reassembled sales-board looks empty');code=patchSource(code);var s=document.createElement('script');s.text=code;document.head.appendChild(s);watchSalesTab();})
    .catch(fail);
})();
