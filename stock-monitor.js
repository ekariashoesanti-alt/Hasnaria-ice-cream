...
(function () {
  ...
  async function postPurchase(id, qty) {
    ...
    var r = await fetch(SB + '/rest/v1/inventory_purchase_log', {
      ...
      body: JSON.stringify({
        brand_id: BRAND, inventory_item_id: id,
        purchase_date: new Date().toISOString().slice(0, 10),
        qty: Number(qty), unit: unit(it),
  ...
  async function load() {
    ...
    try {
      var rows = await get('inventory_items', {
        brand_id: 'eq.' + BRAND, source_period: 'eq.2026-07-01',
      ...
      var logs = await get('inventory_purchase_log', { brand_id: 'eq.' + BRAND, purchase_date: 'eq.' + today, select: 'inventory_item_id,qty' });
      S.buys = {};
      (logs || []).forEach(function (x) { S.buys[x.inventory_item_id] = (S.buys[x.inventory_item_id] || 0) + Number(x.qty || 0); });
    ...
    } catch (e) { S.loading = false; render(e.message); }
  }
  ...
  function stats() {
    var a = 0, b = 0, c = 0;
    S.items.forEach(function (x) { var l = level(x); if (l === 'ok') a++; else if (l === 'order') b++; else c++; });
  ...
  function pages() {
    ...
    if (last <= 7) { for (i = 1; i <= last; i++) out.push(i); return out; }
    out.push(1);
  ...
  function css() {
    ...
    var s = document.createElement('style'); s.id = 'stk-css';
    s.textContent = '#stok{background:transparent!important;border:0!important;padding:0!important;box-shadow:none!important}' +
      ...
      '.stk-wrap *{box-sizing:border-box}.stk-wrap h2{margin:0;font-size:20px;letter-spacing:.04em;font-weight:800;color:#111}' +
      '.stk-sub{margin:6px 0 16px;color:#6b7a74;font-size:13.5px}' +
      '.stk-form{display:grid;grid-template-columns:1.25fr .85fr .85fr 1.25fr .95fr .7fr;gap:14px;align-items:start;background:#fff;border:1px solid #e6ece9;border-radius:14px;padding:16px}' +
      ...
      '.stk-form input[disabled]{background:#f7f9f8;color:#111;font-weight:700}.stk-hint{font-size:11px;color:#8a9690;margin-top:6px}' +
      ...
      '.stk-buy-row input{border:0;height:40px;padding:0;font-size:15px;font-weight:600}.stk-buy-row .unit{font-size:13px;color:#6b7a74}' +
      '.stk-status{height:42px;display:flex;align-items:center}' +
      '.stk-chip{display:inline-flex;align-items:center;padding:4px 10px;border-radius:999px;font-size:12px;font-weight:700}' +
      '.stk-ok{background:#e7f8ee;color:#157a48}.stk-order{background:#fff1d6;color:#b45309}.stk-critical{background:#ffe4e4;color:#b42318}' +
      ...
      '.stk-filters{display:flex;gap:18px;align-items:flex-end;flex-wrap:wrap;margin:12px 0 10px}' +
      ...
      '.stk-seg button{height:34px;padding:0 14px;border:0;background:transparent;font-weight:700;font-size:13px;border-radius:8px;cursor:pointer}' +
      '.stk-seg .on{background:#176b55;color:#fff}.stk-period{height:38px;border:1px solid #d7e0dc;border-radius:10px;padding:0 12px;background:#fff;min-width:140px}' +
      '.stk-xls{margin-left:auto;height:36px;border:1px solid #d7e0dc;background:#fff;border-radius:10px;padding:0 12px;font-weight:700;font-size:13px;color:#176b55;cursor:pointer}' +
      '.stk-table{width:100%;border-collapse:separate;border-spacing:0;font-size:13px}' +
      '.stk-table th{background:#f7f9f8;font-weight:700;padding:10px 8px;border-bottom:1px solid #edf1ef;text-align:center}' +
      '.stk-table td{padding:11px 8px;border-bottom:1px solid #f0f3f1;text-align:center}' +
      ...
      '.stk-pager{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 2px;color:#6b7a74;font-size:13px}' +
      '.stk-pages{display:flex;gap:6px}.stk-pages button{min-width:32px;height:32px;border:1px solid #e2e8e5;background:#fff;border-radius:8px;cursor:pointer}' +
      '.stk-pages .on{background:#176b55;color:#fff;border-color:#176b55}.stk-pages .dots{border:0;background:transparent}' +
      '.stk-foot{margin-top:8px;font-size:12.5px;color:#6b7a74}.stk-info-i{display:inline-flex;width:16px;height:16px;border-radius:99px;border:1px solid #c5d0cb;color:#6b7a74;font-size:11px;margin-left:4px;align-items:center;justify-content:center}' +
      '.stk-gauge{width:100%;max-width:320px;display:block;margin:8px auto 0}.stk-score{text-align:center}.stk-score strong{display:block;font-size:40px;color:#176b55}' +
      '.stk-score .lab{font-size:18px;font-weight:800;color:#176b55}.stk-score small{display:block;color:#6b7a74;font-size:13px}' +
      '.stk-legend{margin-top:16px;border:1px solid #e4ebe8;border-radius:12px;padding:12px 14px;background:#fff}' +
      '.stk-leg{display:flex;justify-content:space-between;padding:6px 0;font-size:13px}.stk-dot{display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:8px}' +
      '.stk-note{margin-top:12px;padding:12px 14px;border:1px solid #bfd6f5;background:#f3f8ff;border-radius:12px;color:#1e4b8c;font-size:13px}' +
  ...
  function render(err) {
    ...
    if (err) { host.innerHTML = html + '<div class="stk-form" style="color:#b42318">'+esc(err)+'</div></div>'; S.drawing=false; return; }
    if (S.loading) { host.innerHTML = html + '<div class="stk-form">Memuat data stok...</div></div>'; S.drawing=false; return; }
    var buy = S.buys[it ? it.id : ''] || '';
    html += '<div class="stk-form"><div><label>Pilih produk</label><select id="stkPick">' +
      S.items.map(function (x) { return '<option value="'+x.id+'"'+(it&&x.id===it.id?' selected':'')+'>'+esc(x.display)+'</option>'; }).join('') +
      '</select></div><div><label>Stok Saat Ini (Qty)</label><input value="'+n(it?current(it):null)+'" disabled><div class="stk-hint">(Per '+todayLabel()+')</div></div>' +
      '<div><label>Minimum (Min)</label><input value="'+n(it?it.min:null)+'" disabled><div class="stk-hint">(Dari data historis)</div></div>' +
      '<div class="stk-buy"><label>Input Pembelian Hari Ini</label><div class="stk-buy-row"><input id="stkBuy" type="number" min="0" value="'+esc(buy)+'" placeholder="0"><span class="unit">'+esc(it?unit(it):'pcs')+'</span></div><div class="stk-hint">Isi jumlah stok baru yang dibeli</div></div>' +
      '<div><label>Order (Qty/pcs/porsi)</label><input value="'+(it?n(it.order)+' '+unit(it):'-')+'" disabled><div class="stk-hint">(Dari data historis)</div></div>' +
      '<div><label>Status</label><div class="stk-status">'+chip(it?level(it):'ok')+'</div></div></div>';
  ...
  function bind() {
    ...
    if (b) b.onchange = async function () {
      var q = Number(this.value); if (!q) return;
      try { await postPurchase(S.selected, q); await load(); } catch (e) { alert(e.message); await load(); }
    };
    var ex = document.getElementById('stkExport');
    if (ex) ex.onclick = exportExcel;
    document.querySelectorAll('#stok [data-mode]').forEach(function (x) { x.onclick = function () { S.mode = x.getAttribute('data-mode'); render(); }; });
    document.querySelectorAll('#stok [data-pg]').forEach(function (x) { x.onclick = function () { S.page = Number(x.getAttribute('data-pg')); render(); }; });
  }
  ...
  function watch() {
    ...
    new MutationObserver(function () {
      if (S.drawing) return;
      if (!hasUi(h)) { if (S.items.length) render(); else if (tok()) load(); }
  ...
  function mount() {
    ...
    if (!h || !ready()) return;
    watch();
    if (!hasUi(h)) { if (S.items.length) render(); else load(); }
  }
  document.addEventListener('click', function (e) {
    ...
    if (id === 'stok' || (t.textContent || '').toLowerCase().indexOf('stok') !== -1) {
      setTimeout(mount, 30); setTimeout(mount, 250);
    }
  }, true);
  var tries = 0;
  (function wait() { mount(); if (tries++ < 400) setTimeout(wait, 250); })();
})();
