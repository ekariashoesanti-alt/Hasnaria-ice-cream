...
function nextPeriod(p){if(!/^\d{4}-\d{2}-\d{2}$/.test(p))return'';var y=+p.slice(0,4),m=+p.slice(5,7);return new Date(Date.UTC(y,m,1)).toISOString().slice(0,10)}
function accountingPeriod(x){var s=String((x&&x.effective_date)||(x&&x.source_period)||'');return /^\d{4}-\d{2}/.test(s)?s.slice(0,7)+'-01':''}
...
function summarize(p){
  var s=blank();
  data.classRows.filter(function(x){return accountingPeriod(x)===p}).forEach(function(x){
...
function rowStatus(x,j){if(isReview(x)&&j)return['Jurnal perlu koreksi','bad'];if(isReview(x))return['Review','warn'];if(!j&&Math.abs(n(x.total_amount))<.01)return['Nilai 0 · tidak dijurnal','warn'];if(!j)return['Belum masuk jurnal','bad'];if(x.debit_account_code&&x.debit_account_code!==j.account_code)return['Akun tidak sinkron','bad'];return[j.status==='provisional'?'Provisional':'Terhubung',j.status==='provisional'?'warn':'ok']}
