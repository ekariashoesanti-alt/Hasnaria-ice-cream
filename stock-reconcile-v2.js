(function () {
  ...
  function ensureCss() {
    ...
    l.rel = 'stylesheet';
    l.href = '/stock-reconcile-v2.css?v=1';
  ...
  function renderStock() {
    ...
    var body = pageRows.map(function (x) {
      ...
      return '<tr><td class="sr-item"><strong>' + esc(x.item_name) + '</strong><small>' + esc(x.unit || 'pcs') + ' · ' + esc(x.category || 'Lainnya') + '</small></td><td class="sr-num sr-plus">+' + fmt(x.purchase_qty) + '</td><td class="sr-num sr-minus">-' + fmt(x.sales_usage_qty) + '</td><td class="sr-num"><strong>' + fmt(x.system_qty) + '</strong></td><td class="sr-num sr-physical">' + phys + '</td><td class="sr-num">' + varianceHtml(x.last_variance) + '</td><td class="sr-num">' + fmt(x.min_qty) + '</td><td>' + chip(x.status) + '</td></tr>';
    }).join('');
    return renderFilters(true) + '<div class="sr-table-card"><div class="sr-table-head"><div><strong>Posisi Stok Saat Ini</strong><span>Penjualan mengurangi stok · Pembelian menambah stok · Opname memvalidasi fisik</span></div><span>' + rows.length + ' item</span></div><div class="sr-table-wrap"><table class="sr-table"><thead><tr><th>Item</th><th>+ Pembelian</th><th>- Penjualan</th><th>Stok Sistem</th><th>Fisik Terakhir</th><th>Selisih</th><th>Min.</th><th>Status</th></tr></thead><tbody>' + (body || '<tr><td colspan="8" class="sr-empty">Tidak ada item sesuai filter.</td></tr>') + '</tbody></table></div>' + renderPager(rows.length, totalPages) + '</div>' +
      '<div class="sr-help-grid"><div class="sr-help-card"><span class="sr-help-label">DARI PENJUALAN</span><strong>Pemakaian otomatis mengurangi stok</strong><p>Jika angka penjualan naik tetapi stok tidak turun, cek mapping produk/BOM di tab Penjualan.</p><button type="button" class="sr-link" data-sr-jump="sales">Cek Penjualan →</button></div><div class="sr-help-card"><span class="sr-help-label">KE KEUANGAN</span><strong>Pembelian adalah arus biaya</strong><p>Catatan pembelian di sini adalah jejak qty persediaan; nominal biaya tetap dikelola di tab Keuangan.</p><button type="button" class="sr-link" data-sr-jump="finance">Cek Keuangan →</button></div><div class="sr-help-card"><span class="sr-help-label">KONTROL</span><strong>Opname menjawab “fisiknya berapa?”</strong><p>Lakukan opname saat stok fisik berbeda dari sistem atau sebelum keputusan pembelian besar.</p><button type="button" class="sr-link" data-sr-tab="opname">Lakukan Opname →</button></div></div><p class="sr-note">Stok Sistem = Baseline + Pembelian − Pemakaian Penjualan. Tracking aktif setelah opname pertama pada item.</p>';
  }
  ...
  function renderPurchase() {
    ...
    var totalQty = S.purchases.reduce(function (a, p) { return a + num(p.qty); }, 0);
    var body = rows.map(function (p) { var it = itemById(p.inventory_item_id); return '<tr><td>' + dateLabel(p.purchase_date) + '</td><td class="sr-item"><strong>' + esc(it ? it.item_name : p.inventory_item_id) + '</strong><small>' + esc(it ? it.category : '') + '</small></td><td class="sr-num"><strong>+' + fmt(p.qty) + '</strong> ' + esc(p.unit || (it && it.unit) || '') + '</td><td>' + esc(p.notes || '-') + '</td></tr>'; }).join('');
    return '<div class="sr-form-card"><div class="sr-section-title"><div><strong>Catat Pembelian</strong><span>Input ini menambah stok. Untuk nominal biaya, lanjutkan di tab Keuangan.</span></div><span class="sr-inline-kpi">' + fmt(totalQty) + ' unit tercatat</span></div><div class="sr-purchase-form"><label>Tanggal<input id="srBuyDate" type="date" value="' + today() + '"></label><label>Item<select id="srBuyItem">' + purchaseItemOptions() + '</select></label><label>Qty<input id="srBuyQty" type="number" min="0.01" step="0.01" placeholder="0"></label><label>Catatan<input id="srBuyNote" type="text" maxlength="120" placeholder="Contoh: supplier / invoice"></label><button type="button" class="sr-btn sr-btn-primary" data-sr-action="save-purchase">Simpan Pembelian</button></div></div><div class="sr-help-card sr-finance-link"><span class="sr-help-label">HUBUNGAN KE KEUANGAN</span><strong>Jangan input biaya pembelian dua kali</strong><p>Tab Stok mencatat <b>qty</b> agar saldo persediaan benar. Tab Keuangan mencatat <b>nilai rupiah</b> agar laporan biaya benar.</p><button type="button" class="sr-link" data-sr-jump="finance">Buka Keuangan →</button></div><div class="sr-table-card"><div class="sr-table-head"><div><strong>Riwayat Pembelian</strong><span>Input terbaru lebih dulu</span></div></div><div class="sr-table-wrap sr-short"><table class="sr-table"><thead><tr><th>Tanggal</th><th>Item</th><th>Qty</th><th>Catatan</th></tr></thead><tbody>' + (body || '<tr><td colspan="4" class="sr-empty">Belum ada pembelian tercatat.</td></tr>') + '</tbody></table></div></div>';
  }
  ...
  function bind(host) {
    host.onclick = async function (e) {
      ...
      if (!t) return;
      if (t.hasAttribute('data-sr-jump')) {
        var target = t.getAttribute('data-sr-jump');
        var id = target === 'sales' ? 'sales' : target === 'finance' ? 'ops' : target;
        var nav = document.querySelector('[data-tab="' + id + '"]') || document.querySelector('[data-page="' + id + '"]');
        if (nav) { nav.click(); return; }
        var legacy = document.getElementById(id);
        if (legacy && legacy.click) legacy.click();
        return;
      }
